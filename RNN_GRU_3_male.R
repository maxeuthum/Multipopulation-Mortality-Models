...
# GRU, 3 layers
GRU3.Groups <- function(T0, tau0, tau1, tau2, tau3, y0=0, optimizer){
  ...
  RNN = Input %>%  
    ...
    layer_gru(units=tau3, activation='tanh', recurrent_activation='tanh', name='GRU3')  # three gru layers of dimensions tau1, tau2, tau3
...
data1 <- data.preprocessing.RNNs(male_mort, 1, T0, tau0, ObsYear)
data2 <- data.preprocessing.RNNs(male_mort, 2, T0, tau0, ObsYear)
data3 <- data.preprocessing.RNNs(male_mort, 3, T0, tau0, ObsYear)
data4 <- data.preprocessing.RNNs(male_mort, 4, T0, tau0, ObsYear)
data5 <- data.preprocessing.RNNs(male_mort, 5, T0, tau0, ObsYear)
data6 <- data.preprocessing.RNNs(male_mort, 6, T0, tau0, ObsYear)
data7 <- data.preprocessing.RNNs(male_mort, 7, T0, tau0, ObsYear)
data8 <- data.preprocessing.RNNs(male_mort, 8, T0, tau0, ObsYear)
data9 <- data.preprocessing.RNNs(male_mort, 9, T0, tau0, ObsYear)
...
##################################################################
male_mort2.group1 <- male_mort[which((male_mort$Year > (ObsYear-T0))&(male_mort$Group_number==1)),] # test data, where 10-year look-back period is needed for recursive prediction, see later
male_mortT.group1 <- male_mort2.group1
test.Y.group1 <- male_mortT.group1[which(male_mortT.group1$Year > ObsYear),]
male_mort2.group2 <- male_mort[which((male_mort$Year > (ObsYear-T0))&(male_mort$Group_number==2)),]
male_mortT.group2 <- male_mort2.group2
test.Y.group2 <- male_mortT.group2[which(male_mortT.group2$Year > ObsYear),]
male_mort2.group3 <- male_mort[which((male_mort$Year > (ObsYear-T0))&(male_mort$Group_number==3)),]
male_mortT.group3 <- male_mort2.group3
test.Y.group3 <- male_mortT.group3[which(male_mortT.group3$Year > ObsYear),]
male_mort2.group4 <- male_mort[which((male_mort$Year > (ObsYear-T0))&(male_mort$Group_number==4)),]
male_mortT.group4 <- male_mort2.group4
test.Y.group4 <- male_mortT.group4[which(male_mortT.group4$Year > ObsYear),]
male_mort2.group5 <- male_mort[which((male_mort$Year > (ObsYear-T0))&(male_mort$Group_number==5)),]
male_mortT.group5 <- male_mort2.group5
test.Y.group5 <- male_mortT.group5[which(male_mortT.group5$Year > ObsYear),]
male_mort2.group6 <- male_mort[which((male_mort$Year > (ObsYear-T0))&(male_mort$Group_number==6)),]
male_mortT.group6 <- male_mort2.group6
test.Y.group6 <- male_mortT.group6[which(male_mortT.group6$Year > ObsYear),]
male_mort2.group7 <- male_mort[which((male_mort$Year > (ObsYear-T0))&(male_mort$Group_number==7)),]
male_mortT.group7 <- male_mort2.group7
test.Y.group7 <- male_mortT.group7[which(male_mortT.group7$Year > ObsYear),]
male_mort2.group8 <- male_mort[which((male_mort$Year > (ObsYear-T0))&(male_mort$Group_number==8)),]
male_mortT.group8 <- male_mort2.group8
test.Y.group8 <- male_mortT.group8[which(male_mortT.group8$Year > ObsYear),]
male_mort2.group9 <- male_mort[which((male_mort$Year > (ObsYear-T0))&(male_mort$Group_number==9)),]
male_mortT.group9 <- male_mort2.group9
test.Y.group9 <- male_mortT.group9[which(male_mortT.group9$Year > ObsYear),]
...
pred.rates.male <- array(NA, dim=c(9*xx, number_loops)) # matrix of predicted years for several run-throughs
forecasted.rates.male <- array(NA, dim=c(9*46*4, number_loops)) # matrix of forecasted logrates, 4 years and 9 groups
in.sample.error.male <- rep(0, number_loops) # in-sample error for each loop  to be directly calcualted in loop to have a first performance impression
out.sample.error.male <- rep(0, number_loops) # out-sample error for each loop
error.male <- rep(0,9) # store out-sample errors in each loop
...
RNN.type <- "GRU" # selected RNN-type
...
for (i in 1:number_loops) {
  ...
  model <- GRU3.Groups(T0, tau0, tau1, tau2, tau3, y0, optimizer)
  ...
  pred.rates.male[,i] <- model %>% predict(x.train) # sorted by year, age, group
  ...
  # Group 1
  pred.result.group1 <- recursive.prediction.Group(ObsYear, male_mort2.group1, 1, T0, tau0, x.min, x.max, model)
  test.group1 <- pred.result.group1[[1]][which(male_mort2.group1$Year > ObsYear),] # forecasted logrates
  error.male[1] <- round(10^4*mean((exp(test.group1$Lograte)-exp(test.Y.group1$Lograte))^2),4) # out-sample error
  
  # Group 2
  pred.result.group2 <- recursive.prediction.Group(ObsYear, male_mort2.group2, 2, T0, tau0, x.min, x.max, model)
  test.group2 <- pred.result.group2[[1]][which(male_mort2.group2$Year > ObsYear),]
  error.male[2] <- round(10^4*mean((exp(test.group2$Lograte)-exp(test.Y.group2$Lograte))^2),4)
  
  # Group 3
  pred.result.group3 <- recursive.prediction.Group(ObsYear, male_mort2.group3, 3, T0, tau0, x.min, x.max, model)
  test.group3 <- pred.result.group3[[1]][which(male_mort2.group3$Year > ObsYear),]
  error.male[3] <- round(10^4*mean((exp(test.group3$Lograte)-exp(test.Y.group3$Lograte))^2),4)
  
  # Group 4
  pred.result.group4 <- recursive.prediction.Group(ObsYear, male_mort2.group4, 4, T0, tau0, x.min, x.max, model)
  test.group4 <- pred.result.group4[[1]][which(male_mort2.group4$Year > ObsYear),]
  error.male[4] <- round(10^4*mean((exp(test.group4$Lograte)-exp(test.Y.group4$Lograte))^2),4)
  
  # Group 5
  pred.result.group5 <- recursive.prediction.Group(ObsYear, male_mort2.group5, 5, T0, tau0, x.min, x.max, model)
  test.group5 <- pred.result.group5[[1]][which(male_mort2.group5$Year > ObsYear),]
  error.male[5] <- round(10^4*mean((exp(test.group5$Lograte)-exp(test.Y.group5$Lograte))^2),4)
  
  # Group 6
  pred.result.group6 <- recursive.prediction.Group(ObsYear, male_mort2.group6, 6, T0, tau0, x.min, x.max, model)
  test.group6 <- pred.result.group6[[1]][which(male_mort2.group6$Year > ObsYear),]
  error.male[6] <- round(10^4*mean((exp(test.group6$Lograte)-exp(test.Y.group6$Lograte))^2),4)
  
  # Group 7
  pred.result.group7 <- recursive.prediction.Group(ObsYear, male_mort2.group7, 7, T0, tau0, x.min, x.max, model)
  test.group7 <- pred.result.group7[[1]][which(male_mort2.group7$Year > ObsYear),]
  error.male[7] <- round(10^4*mean((exp(test.group7$Lograte)-exp(test.Y.group7$Lograte))^2),4)
  
  # Group 8
  pred.result.group8 <- recursive.prediction.Group(ObsYear, male_mort2.group8, 8, T0, tau0, x.min, x.max, model)
  test.group8 <- pred.result.group8[[1]][which(male_mort2.group8$Year > ObsYear),]
  error.male[8] <- round(10^4*mean((exp(test.group8$Lograte)-exp(test.Y.group8$Lograte))^2),4)
  
  # Group 9
  pred.result.group9 <- recursive.prediction.Group(ObsYear, male_mort2.group9, 9, T0, tau0, x.min, x.max, model)
...
mean.pred.rates.male <- apply(pred.rates.male, 1, mean)
mean.forecasted.rates.male <- apply(forecasted.rates.male, 1, mean)
mean.in.sample.error.male <- mean(in.sample.error.male)
mean.out.sample.error.male <- mean(out.sample.error.male)

# order forecasted logrates to get same order as for predicted logrates from training data
final.forecasted.rates.male <- array(NA, dim = 9*46*4)
...
plot.losses <- function(name.model, gender, val_loss, loss){
  ggplot(aes(x=epochs,y=val_loss), data = val_loss) + geom_point(color = "chartreuse4") +
    ggtitle(list(paste("early stopping: ",name.model,", ", gender, sep=""), cex=1.5)) +
    xlab("Epochs") + ylab("MSE loss") + 
